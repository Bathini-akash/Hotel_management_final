...
from customer.models import Member,Guest,MemberPhone,MemberPass
...
def book_now(request):
	...
	p2=p.price
	p=Room_type.objects.get(rtype='C')
	p3=p.price
...
def book_confirm(request):
	...
	if types=="guest":
		...
		user=Member.objects.get(memail=username)
		c=MemberPhone.objects.get(mid=user.mid)
		customer=Guest(fname=user.fname,lname=user.lname,gemail=username,phone1=c.phone1,phone2=c.phone2,mid=user)
		customer.save()
		if user.level=='G' or user.level=='P':
			data=Booking(gid=customer,plantype='C',hid=ho,checkin=start_date,checkout=end_date,cost=amount)
			data.save()
			request.session['bid']=data.book_no
			book_no=data.book_no
		else:
			data=Booking(gid=customer,plantype='B',hid=ho,checkin=start_date,checkout=end_date,cost=amount)
			data.save()		
			request.session['bid']=data.book_no
			book_no=data.book_no		
	...
	for r in range(int(room1)):
		...
		if service1!="Not availed" and serv1!=0:
			ser=Service.objects.get(sname=service1[0])
			data2=Booking_detail(book_no=data,book_dtno=dt,rtype=rt,No_adults=temp1,No_children=temp2,sid=ser)
			data2.save()
			serv1-=1
			service1.pop(0)
		else:	
	...
	for r in range(int(room2)):
		...
		temp1=math.trunc(temp1)
		temp2=math.trunc(temp2)
		if service2!="Not availed" and serv2!=0:
			ser=Service.objects.get(sname=service2[0])
			data2=Booking_detail(book_no=data,book_dtno=dt,rtype=rt,No_adults=temp1,No_children=temp2,sid=ser)
			data2.save()
			serv2-=1
			service2.pop(0)
	...
	for r in range(int(room3)):
		...
		temp1=math.trunc(temp1)
		temp2=math.trunc(temp2)	
		if service3!="Not availed" and serv3!=0:
			ser=Service.objects.get(sname=service3[0])
			data2=Booking_detail(book_no=data,book_dtno=dt,rtype=rt,No_adults=temp1,No_children=temp2,sid=ser)
			data2.save()
			serv3-=1
			service3.pop(0)
	...
	if request.method=="POST":
		...
		if not len(username):
			...
			if Member.objects.filter(memail=username):
				...
				c=user.mid
				temp=MemberPass.objects.filter(mid=c)[0]
				password_hash=temp.password
				print(password_hash)
				res=check_password(password_hash,password)
        ...
        if Member.objects.filter(memail=email):
           messages.warning(request,"Account already exist, please Login to continue")
            ...
            if(len(error)==0):
                ...
                customer=Member(fname=fname,lname=lname,memail=email,level='B',profile_pic=profile_pic)
                customer.save()
                data=MemberPhone(mid=customer,phone1=phone1,phone2=phone2)
                data.save()
                data=MemberPass(mid=customer,password=password)
                data.save()
